# Description: Utility functions for the auto punctuation package
...
class FixPunctuation:
    ...
    #TODO add support for cuda usage ✅
    #TODO add support to turn off cuda usage even if available
    def __init__(self, total_words_per_pred=250):
        self.total_words_per_pred = total_words_per_pred
        self.overlap_wrds = 30
        self.valid_labels = ['OU', 'OO', '.O', '!O', ',O', '.U',
                             '!U', ',U', ':O', ';O', ':U', "'O",
                             '-O', '?O', '?U']
        self.model = NERModel("bert", "speeqo/bert-restore-punctuation", labels=self.valid_labels,
                              args={"silent": True, "max_seq_length": 512}, use_cuda=is_available())
    ...
    def punctuate(self, text: str, lang:str=''):
        ...
        splits = self.split_on_toks(text, self.total_words_per_pred, self.overlap_wrds)
        # predict slices
